...
namespace compound {
Window::Window(const Init& init, int width, int height, const std::string& title) : m_surface(0) {
    LOG4CPLUS_INFO(m_logger, "Creating window");
    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    if ((m_handle = glfwCreateWindow(width, height, title.c_str(), 0, 0)) == nullptr) {
        LOG4CPLUS_ERROR(m_logger, "Failed to create window");
        throw std::runtime_error("Failed to create window");
    }
    VkSurfaceKHR surface;
    if (glfwCreateWindowSurface(*init.getVkInstance(), m_handle, 0, &surface) != VK_SUCCESS) {
